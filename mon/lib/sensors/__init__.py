...
SENSORS_LIB_FILENAME = os.environ.get('SENSORS_LIB') or find_library('sensors')
if SENSORS_LIB_FILENAME is None:
    print "SENSORS LIB NOT FOUND. PLEASE DEFINE SENSORS_LIB"
    exit()
SENSORS_LIB = CDLL(SENSORS_LIB_FILENAME)
...
class Feature(Structure):
    ...
    @property
    def label(self):
        label_p = _get_label(byref(self.chip), byref(self))
        result = ''.join(c_char_p(label_p).value)
        stdc.free(label_p)
        return result
